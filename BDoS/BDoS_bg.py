...
def helptext ():
        print "Помощ: %s е 1.0 за GNU/Linux - <сайт> [-t] [-c] [-p]\n" % sys.argv[0]
        print " пример python2 BDoS.py www.website.com -t 1000 -c 1000 -p 80"
        print ""
        print " HTTP method"
        print ""
        print "   -t\t\tНомер ботове"
        print "   -c\t\tВръзка връзки на бот"
        print "   -p\t\tПорт"
        print ""
        print "\n"
...
def main (args):
        ...
        while i < len(args):
                ...
                if a == "--help":
                        ...
                        print "Грешен аргумент '%s'" % args[i]
                        exit()
        ...
        while True:
                ...
                for i in range (connection_amount):
                        ...
                        try:
                                ...
                                col()
                                print " ГРЕШКА В БОТ %d: НЕ МОЖЕ ДА СЕ СВЪРЖЕ" % num
                                bleh = True
                if bleh:
                        continue

                col()
                print " Бот %d отвори %d връзки" % (num, connection_amount)
                ...
                while i < cap and not bleh:
                        ...
                        for c in cons:
                                try:
                                        ...
                                        bleh = True
                        col()
                        print " Бот %d изпрати информация (%d/%d)" % (num, i, cap)
                        time.sleep(3)
                ...
                if bleh:
                        for c in cons:
                                try:
                                        ...
                                        pass
                        col()
                        print " :( Бот %d умря, рестартиране" % num
                        cons = []
                        continue
                ...
                cons = []
                col()
                print " Бот %d затвори всички връзки" % num
...
def start ():
        ...
        print " Атакатувате %s%s%s със порт %s%d%s ползвайки %s%d%s ботове" % (h_on, host, h_off, h_on, port, h_off, h_on, num_threads, h_off)
        ...
        try:
                ...
                print "\n\n Спиране на програмата > <Ctrl - C> хванато\n"
                exit()
